<template>
  <div class="h-screen flex flex-col">
    <div class="h-14 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center px-4 gap-3 shrink-0">
      ...
      <a :href="downloadUrl" class="btn-secondary !text-xs !py-1 ml-auto" download>{{ $t('file.download') }}</a>
    </div>
...
onMounted(async () => {
  try {
    ...
    previewType.value = getPreviewType(res.data.mime_type);
    previewUrl.value = `/api/files/${fileId}/download`;
    downloadUrl.value = `/api/files/${fileId}/download`;

    if (previewType.value === 'code') {
      const downloadRes = await fetch(previewUrl.value);
      textContent.value = await downloadRes.text();
    }
  } catch (e) {
